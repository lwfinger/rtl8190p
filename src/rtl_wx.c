		...
		union iwreq_data *wrqu, char *extra)
{
	...
	//printk("Start Log: Set 0x08000000 to 0x230\n");
	...
	//printk("0xf0: %2.2x\n0xf4: %2.2x\n0x140: %2.2x\n0x124: %2.2x\n", reg1,reg2,reg3,reg4);
	//printk("After delay 10ms, read 0x230: %8.8x\n", reg5);
	len = strlen(extra);
...
{
	//memset(extra, 0, 64);
	//sprintf(extra,"\nStart Log: Try to resume firmware\n");
			     ...
			     union iwreq_data *wrqu, char *b)
{
	...
	{
		//printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!%s:In mesh mode and mesh state is linked!\n", __FUNCTION__);
		return 0;
	}
#endif
	//YJ,add,090216,end
...
#ifdef _RTL8192_EXT_PATCH_
		rtllib_softmac_scan_syncro(priv->rtllib, 0);
#else
		...
		if(ieee->state == RTLLIB_LINKED){
			if(ieee->iw_mode == IW_MODE_ADHOC)
			{
				...
				setKey( dev,
						key_idx,                      //EntryNo
						key_idx,                      //KeyIndex
						ieee->pairwise_key_type,        //KeyType
						zero_addr[key_idx],		//addr
						0,                      //DefaultKey
						hwkey);                 //KeyContent
				set_swcam( dev,
						key_idx,                      //EntryNo
...
int rtl8192_set_key_for_AP(struct rtllib_device *ieee)
{
	...
	int key_len=0;
	char key[32]; //max keylen of tkip,ccmp wep
	u16 keytype = IW_ENCODE_ALG_NONE;
#ifdef _RTL8192_EXT_PATCH_
	crypt = &ieee->sta_crypt[ieee->tx_keyidx];// AMY modify 090226
#else
	...
	set_swcam( ieee->dev,
				31,//EntryNo
				ieee->tx_keyidx, //KeyIndex
				keytype,  //KeyType
				ieee->ap_mac_addr, //MacAddr
				0,              //DefaultKey
				(u32 *)key ,          //KeyContent
				0);
	setKey( ieee->dev,
                        ...
                        struct iw_point *encoding, struct iw_encode_ext *ext, u8 *addr)
{
	...
	{
		if ((i=rtllib_find_MP(ieee, addr, 0)) < 0) //crypt entry is not exist before
		{
			i = rtllib_find_MP(ieee, addr, 1); //set entry and get index.
			if (i<0)
				return -1;
		}
	}
...

	{
		...
		{
			ieee->mesh_group_key_type = alg;
#if 0//Don't set gtk,just use sw decrypt.
			setKey( dev,
					idx,//EntryNo
					idx, //KeyIndex
					alg,  //KeyType
...
#endif
		if((ieee->only_mesh == 0) &&(ieee->is_server_eth0 == 0))  //YJ,add,090217
		{
...
printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!set current mesh network channel %d\n", ch);
		...
		//YJ,add,090602
		if((ieee->meshscanning) || (ieee->be_scan_inprogress))
...
u16 rtl8192_11n_user_show_rates(struct net_device* dev)
{
	...
	u8  rate = MGN_1M;
	u32  Sgstrength;
	bool TxorRx = priv->rtllib->bForcedShowRxRate;          //false:Tx   true:Rx

	// Forced data rate: Tx rate is forced to a constant value.
	//if(priv->ForcedDataRate != 0)
	//      return CONVERT_RATE(priv->rtllib, priv->ForcedDataRate);

	// Some special handling for Tx rate indication.
	// Tx rate
	if (!TxorRx) {
		//if(priv->OSTxRateDisplayType == TxRateTypeCurrent)
		//{
		// Report the successful transmit rate culculate from dm_CheckStatistics().
		// This value refresh every 2 seconds in checkforhang function.
		//return 0;
		//}
		//else if(priv->OSTxRateDisplayType == TxRateTypeStartRate)
		{
			// Report initial rate in HW. This value is also refresh in dm_CheckStatistics() every 2 seconds.
	...
	if (priv->rtllib->mode == WIRELESS_MODE_A ||
		...
		u8    rftype = priv->rf_type;//Adapter->HalFunc.HalGetRFTypeHandler(Adapter);
		//Adapter->HalFunc.GetHwRegHandler(Adapter, HW_VAR_1X1_RECV_COMBINE, (pu1Byte)(&b1SSSupport));
		...
		// Return rate Max rate for the first 100 times query for throughput issue.
		if(priv->rtllib->state != RTLLIB_LINKED)
		...
		if (TimesForReportingFullRxSpeedAfterConnected > priv->rtllib->SystemQueryDataRateCount) {
			...
			if(bMaxRateMcs15)
				return 600;//HTMcsToDataRate(priv->rtllib, MGN_MCS15);
			else
				return 270;//HTMcsToDataRate(priv->rtllib, MGN_MCS7);
		}
		...
		//RT_TRACE(COMP_OID_QUERY, ("MgntActQuery_RT_11N_USER_SHOW_RATES():rate(%#x)\n",rate));
...
#define MAX_NEIGHBOR_NUM 64 //YJ,modified,090304
		                            ...
		                            u16 KeyType, u32 *KeyContent )
{
	...
	for(i=0; i<pmesh_neighbor->num; i++)
	{
		...
		set_swcam( dev,
				...
				KeyIndex, //KeyIndex
				KeyType,  //KeyType
				pneighbor_entry->MacAddr, //MacAddr
				0,              //DefaultKey
			     ...
			     union iwreq_data *wrqu, char *key)
{
	...
	int i;
       ...
       if(!priv->mesh_up) return -ENETDOWN;//AMY modify 090220
	...
	if(wrqu->encoding.length!=0){
		...
		switch(wrqu->encoding.flags & IW_ENCODE_INDEX){
			case 0: key_idx = ieee->mesh_txkeyidx; break;//AMY modify 090226
			case 1:	key_idx = 0; break;
			case 2:	key_idx = 1; break;
			case 3:	key_idx = 2; break;
			case 4:	key_idx	= 3; break;
		...
		meshdev_set_key_for_linked_peers(dev,
						key_idx, //KeyIndex
						ieee->mesh_pairwise_key_type,  //KeyType
						hwkey); //KeyContent
...
static iw_handler meshdev_wx_handlers[] =
{
	...
	meshdev_wx_set_mlme, //NULL,    /* SIOCSIWMLME*/                 /* -- hole -- */
	dummy,                     /* SIOCGIWAPLIST -- depricated */
	NULL,  //meshdev_wx_set_scan,        /* SIOCSIWSCAN */
	NULL,  //meshdev_wx_get_scan,        /* SIOCGIWSCAN */
	...
	NULL,                     /* -- hole -- */
	NULL,                     /* -- hole -- */
	NULL,  //meshdev_wx_set_rate,        /* SIOCSIWRATE */
	meshdev_wx_get_rate,        /* SIOCGIWRATE */
	dummy,                    /* SIOCSIWRTS */
	dummy,                    /* SIOCGIWRTS */
	NULL,  //meshdev_wx_set_frag,        /* SIOCSIWFRAG */
	NULL,  //meshdev_wx_get_frag,        /* SIOCGIWFRAG */
	dummy,                    /* SIOCSIWTXPOW */
	dummy,                    /* SIOCGIWTXPOW */
	NULL,  //meshdev_wx_set_retry,       /* SIOCSIWRETRY */
	NULL,  //meshdev_wx_get_retry,       /* SIOCGIWRETRY */
	...
	NULL,			/*---hole---*/
	NULL,			/*---hole---*/
	meshdev_wx_set_gen_ie,//NULL,			/* SIOCSIWGENIE */
	NULL,			/* SIOCSIWGENIE */
	meshdev_wx_set_auth,//NULL,			/* SIOCSIWAUTH */
	NULL, //r819x_wx_get_auth,//NULL,			/* SIOCSIWAUTH */
	meshdev_wx_set_enc_ext,				/* SIOCSIWENCODEEXT */
	NULL, //r819x_wx_get_enc_ext,//NULL,			/* SIOCSIWENCODEEXT */
	(iw_handler) rt_ioctl_siwpmksa,//NULL,			/* SIOCSIWPMKSA */
	NULL,			 /*---hole---*/
};
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	if(*extra == 0)
	{
		...
		if(ieee->state!=RTLLIB_LINKED){//AMY test 090217
			if(ieee->pHTInfo->bCurBW40MHz)
				HTSetConnectBwMode(ieee, HT_CHANNEL_WIDTH_20_40, (ieee->current_mesh_network.channel<=6)?HT_EXTCHNL_OFFSET_UPPER:HT_EXTCHNL_OFFSET_LOWER);  //20/40M
			else
				HTSetConnectBwMode(ieee, HT_CHANNEL_WIDTH_20, (ieee->current_mesh_network.channel<=6)?HT_EXTCHNL_OFFSET_UPPER:HT_EXTCHNL_OFFSET_LOWER);  //20M
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	//YJ,tmp add for script test,090723
...
static int Set_ExtChnOffset_Proc(struct net_device *meshdev, char *arg)
{
	struct meshdev_priv *mpriv = (struct meshdev_priv *)netdev_priv_rsl(meshdev);
	struct rtllib_device *ieee = mpriv->rtllib;
//YJ,del,090821,since there is no HT info in beacon.
#if 0
	struct r8192_priv *priv = (void *)ieee->priv;
	struct net_device *dev = ieee->dev;
	struct mshclass *mshobj= priv->mshobj;
...
static iw_handler meshdev_private_handler[] = {
	meshdev_wx_enable_mesh,		//SIOCIWFIRSTPRIV + 0x0
	meshdev_wx_get_meshinfo,		//SIOCIWFIRSTPRIV + 0x1
	meshdev_wx_disable_mesh,		//SIOCIWFIRSTPRIV + 0x2
	meshdev_wx_set_channel,			//SIOCIWFIRSTPRIV + 0x3
	meshdev_wx_set_meshid,			//SIOCIWFIRSTPRIV + 0x4
	meshdev_wx_get_mesh_list,		//SIOCIWFIRSTPRIV + 0x5
	meshdev_wx_mesh_scan,			//SIOCIWFIRSTPRIV + 0x6
	meshdev_wx_set_meshmode,		//SIOCIWFIRSTPRIV + 0x7
	meshdev_wx_set_hostname,		//SIOCIWFIRSTPRIV + 0x8
	meshdev_wx_set_meshbw,			//SIOCIWFIRSTPRIV + 0x9
	meshdev_wx_update_beacon,		//SIOCIWFIRSTPRIV + 0xA
	meshdev_wx_get_mac_deny,		//SIOCIWFIRSTPRIV + 0xB
	meshdev_wx_add_mac_deny,		//SIOCIWFIRSTPRIV + 0xC
	NULL,					//SIOCIWFIRSTPRIV + 0xD
	NULL,//meshdev_wx_mesh,			//SIOCIWFIRSTPRIV + 0xE
	meshdev_wx_get_neighbor_list,		//SIOCIWFIRSTPRIV + 0xF
	meshdev_wx_set_param,			//SIOCIWFIRSTPRIV + 0x10
	NULL,					//SIOCIWFIRSTPRIV + 0x11
	meshdev_wx_del_mac_deny,		//SIOCIWFIRSTPRIV + 0x12
	NULL,							//SIOCIWFIRSTPRIV + 0x13
	meshdev_wx_set_mesh_security,		//SIOCIWFIRSTPRIV + 0x14
	meshdev_wx_set_mkdd_id,			//SIOCIWFIRSTPRIV + 0x15
	meshdev_wx_set_mesh_key,			//SIOCIWFIRSTPRIV + 0x16
	meshdev_wx_set_sec_type,		//SIOCIWFIRSTPRIV + 0x17
};
